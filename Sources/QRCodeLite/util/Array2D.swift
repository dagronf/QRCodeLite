...
extension Array2D {
	...
	func flipVertical() -> Array2D<T> {
		let flipped = stride(from: 0, to: self.rows * self.columns, by: self.columns)
			.reversed()
			.flatMap { offset in
				self.array[offset ..< offset + self.rows]
			}
		return Array2D(rows: self.rows, columns: self.columns, flattened: flipped)
	}

	/// Flip the content horizontally
	func flipHorizontally() -> Array2D<T> {
		let flipped = stride(from: 0, to: self.rows * self.columns, by: self.columns)
			.flatMap { offset in
				self.array[offset ..< offset + self.rows].reversed()
			}
		return Array2D(rows: self.rows, columns: self.columns, flattened: flipped)
	}
}
